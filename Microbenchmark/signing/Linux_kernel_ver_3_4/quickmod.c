...
static __u64 mac_core( const char *log_msg, const int msg_len,  __u64 * proof)
{
	...
	if (remaining){//last block + generating new key
		...
		* pad_header = counter;
		...
		cipher_blks[1] = xor_block(cipher_blks[1], _mm_setr_epi32(0x0001, 0x0000, 0x0000, 0x0000));/*1 for updatting key*/
...
#endif
	tmper = _mm_extract_epi64(tag_blks[2], 0);
	*proof = tmper;
...
static u64 sign_event(char *log_msg, siphash_key_t sign_key,size_t key_len)
{
	...
	integrity_proof = siphash(log_msg, log_msg_len, &sign_key);
...
/*************************************QuickLog*************************************************/	
	...
	for(i=0;i<10;i++){
		...
		{	
			...
			kernel_fpu_begin();
			quick_tag = mac_core(str, len, integrity_proof);
			kernel_fpu_end();
